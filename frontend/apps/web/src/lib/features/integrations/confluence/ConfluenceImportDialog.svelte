...
<Dialog.Root {openController}>
  <Dialog.Content width="medium">
    <Dialog.Title>Import knowledge from Confluence</Dialog.Title>
      ...
      <div class="flex flex-grow flex-col gap-1 rounded-md p-4">
        ...
        <div class="relative flex flex-grow">
          <input
            ...
            placeholder="Find confluence space"
            {...$input}
            required
            use:input
        ...
        >
          ...
          <div class="bg-primary text-primary flex flex-col gap-0" tabindex="0">
            ...
            {:else if filteredResources.length > 0}
              ...
              {/each}
            {:else}
              <span class="text-secondary px-2 py-1">No matching spaces found.</span>
            {/if}
          </div>
        </ul>
      </div>
    ...
    <Dialog.Controls>
      ...
      >
        {importKnowledge.isLoading ? "Importing..." : "Import space"}
      </Button>
    </Dialog.Controls>
  </Dialog.Content>
</Dialog.Root>
