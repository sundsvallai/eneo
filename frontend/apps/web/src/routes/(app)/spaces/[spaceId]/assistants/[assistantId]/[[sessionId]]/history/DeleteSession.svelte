<script lang="ts">
  ...
  export let session: Omit<AssistantSession, "messages">;
  ...
  async function deleteSession(session: { id: string }) {
    await intric.assistants.deleteSession({ assistant, session });
...
<div class="flex items-center justify-end">
  <Dialog.Root alert>
    <Dialog.Trigger asFragment let:trigger>
      <Button destructive is={trigger} label="Delete session" padding="icon">
        <svg
