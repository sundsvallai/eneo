    ...
    <Dialog.Section>
      ...
      >
        <div class="flex items-baseline justify-between">
          Role name<span class="px-2 text-[0.9rem] font-normal text-stone-400"
            >A descriptive name for this role</span
          >
        </div></Input.Text
      >
      <div class="px-4 py-4">
        <div class="flex items-baseline justify-between pb-2 pl-3 font-medium">
          Included permissions<span class="px-2 text-[0.9rem] font-normal text-stone-400"
            >What users of this role can manage</span
          >
        </div>
        <div class="overflow-clip rounded-md border border-stone-300 bg-stone-100">
          {#each permissions as permission}
            <div
              class="flex flex-col gap-1 border-b border-stone-200 px-4 py-4 last-of-type:border-b-0 hover:bg-stone-50"
            >
              <Input.Switch
                {disabled}
                class="capitalize"
              ...
              >
              <p class="text-[0.9rem] text-stone-500">{permission.description}</p>
            </div>
          {/each}
        </div>
      </div>
