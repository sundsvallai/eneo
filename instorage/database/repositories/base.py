...
class BaseRepositoryDelegate:
    ...
    def _get_query_for_related(self, table: Type[Base], id_list: list[ModelUUID]):
        _ids = [item.id for item in id_list]

        return sa.select(table).filter(table.uuid.in_(_ids))

    async def _get_related(
        self,
        table: Type[Base],
        id_list: list[ModelUUID],
    ...
    async def update(
        ...
        **extra_kwargs
        ...
        if "id" in new_entry.model_dump(exclude_unset=True):
            query = query.where(self.table.id == new_entry.id)
        else:
            query = query.where(self.table.uuid == new_entry.uuid)
    ...
    async def get(self, id: int, user_id: int = None):
    ...
    async def delete_by_uuid(self, uuid: UUID):
        return await self.delete_by(conditions={self.table.uuid: uuid})
